...
from gala.data.embodiment_tags import EMBODIMENT_TAG_MAPPING, EmbodimentTag
...
def _make_bridge_tokens(num_tokens: int) -> list:
    ...
    the authoritative value is the model's ``bridge_cfg.num_bridge_tokens``.
    """
    if not isinstance(num_tokens, int) or num_tokens <= 0:
        raise ValueError(
            ...
            "(``bridge_cfg.num_bridge_tokens``); upstream code must pass it explicitly."
        )
    ...
    for key in keys:
        ...
        if key == "eagle_content":
            ...
            for v in values:
                ...
                sample_image_counts.append(len(curr_image_inputs))
            # print(text_list)
            # print(image_inputs)
            ...
            for k, v in eagle_inputs.items():
                ...
                batch[k] = v
                # print(k, v.shape)
            batch["eagle_sample_image_counts"] = torch.tensor(sample_image_counts, dtype=torch.long)
        ...
        elif key in ("orig_state", "dexlam_language"):
            batch[key] = values
        else:
...
class GR00TTransform(InvertibleModalityTransform):
    ...
    # (``bridge_cfg.num_bridge_tokens``); upstream code reads it from there
    ...
    def model_post_init(self, __context):
        ...
        if self.use_bridge and self.num_bridge_tokens is None:
            raise ValueError(
                ...
                "Read it from the model config (bridge_cfg.num_bridge_tokens) "
