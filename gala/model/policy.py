...
from gala.data.embodiment_tags import EmbodimentTag
from gala.data.schema import DatasetMetadata
from gala.data.transform.base import ComposedModalityTransform
#use the latent bridge (cotrain version with unified_embodiment_id support)
from gala.model.gala import GALAModel
...
class GALAPolicy(BasePolicy):
    ...
    def __init__(
        ...
        dinov2_path_override: Optional[str] = None,
        ...
        Args:
            ...
            dinov2_path_override (Optional[str]): Optional path to override DINOv2 model location
                                                   in tokenizer (for deployment).
        """
        try:
        ...
        if not math.isfinite(infer_sample_shift) or infer_sample_shift <= 0:
            raise ValueError(f"infer_sample_shift must be finite and > 0, got {infer_sample_shift}")
        self.model.action_head.infer_sample_shift = float(infer_sample_shift)
        print(f"Set action inference sample shift to {infer_sample_shift}")
    ...
    def _get_unnormalized_action(self, normalized_action: torch.Tensor, normalized_input: Dict[str, Any]) -> Dict[str, Any]:
        # HierarchicalRelativeTransform.unapply needs full tensors including state; keep a copy
        # so fields like orig_state remain for inverse transforms.
        ...
        for k, v in data_to_unapply.items():
            if isinstance(v, torch.Tensor):
                ...
                # Matrices in transforms expect float64 tensors.
    ...
    def _load_model(self, model_path, tokenizer_len=None, compute_bridge_loss=False, dinov2_path_override=None):
        # print("loading model ...")
        model_cls = GALAModel
        ...
        if expected_action_horizon != model.action_head.config.action_horizon:
            ...
            from gala.model.action_head.flow_matching_action_head import (
                FlowmatchingActionHeadGALA,
            )

            # Create new action head with updated config
            new_action_head = FlowmatchingActionHeadGALA(new_action_head_config)
