...
from gala.data.transform.base import ModalityTransform
...
class VideoOffsetCrop(VideoTransform):
    """
    Crop video frames to a fixed rectangle (same offset/size for every frame).
    """
    top: int = Field(..., description="Top pixel of the crop region.")
    left: int = Field(..., description="Left pixel of the crop region.")
    ...
    def get_transform(self, mode: Literal["train", "eval"] = "train") -> Callable:
        """
        Return a torchvision crop applied per-frame to tensors shaped (T, C, H, W).
        """
        if self.backend == "torchvision":
            # F.crop accepts (..., H, W); applies the same crop to each frame in T.
