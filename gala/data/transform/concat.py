...
from gala.data.schema import DatasetMetadata, StateActionMetadata
from gala.data.transform.base import InvertibleModalityTransform
...
class ConcatTransform(InvertibleModalityTransform):
    ...
    def apply(self, data: dict) -> dict:
        ...
        if "state" in grouped_keys:
            ...
            # Store the original state keys that are not concatenated
            # print(f"grouped_keys['state']: {grouped_keys['state']}")
            # print(f"state_concat_order: {self.state_concat_order}")
            orig_state = {}
            for key in grouped_keys["state"]:
                if key not in self.state_concat_order:
                    orig_state[key] = data[key].clone()
            data["orig_state"] = orig_state
    ...
    def _get_unapply_dim(self, key: str, base_dim: int) -> int:
        """If rotation key with 3D base, assume it was converted to 6D."""
        if self.is_rotation_key(key) and base_dim == 3:
            return 6
        return base_dim
    ...
    def unapply(self, data: dict) -> dict:
        ...
        for key in self.action_concat_order:
            ...
            dim = self._get_unapply_dim(key, self.action_dims[key])
        ...
        if "state" in data:
            ...
            for key in self.state_concat_order:
                dim = self._get_unapply_dim(key, self.state_dims[key])
                end_dim = start_dim + dim
                data[key] = state_tensor[..., start_dim:end_dim]
                start_dim = end_dim
        # Restore passthrough original state keys from orig_state.
        orig_state = data.pop("orig_state", {})
        for key in orig_state:
            data[key] = orig_state[key]
        return data
