...
import robocasa  # Registers GR1 environments.
import robosuite
...
from gala.eval.service import BaseInferenceClient
from gala.eval.wrappers.multistep_wrapper import MultiStepWrapper
from gala.eval.wrappers.video_recording_wrapper import (
    VideoRecorder,
    VideoRecordingWrapper,
)
from gala.model.policy import BasePolicy
...
class SimulationConfig:
    ...
    # Optional set of "state.*" modality keys expected by the policy. When non-empty, the
    # video/multistep wrappers will reconcile env observations against this key set
    # (see VideoRecordingWrapper / MultiStepWrapper). Empty set keeps legacy behavior.
    state_modality_keys: set = field(default_factory=set)
...
def _create_single_env(config: SimulationConfig, idx: int) -> gym.Env:
    ...
    if config.video.video_dir is not None:
        ...
        env = VideoRecordingWrapper(
            ...
            state_modality_keys=config.state_modality_keys,
        )
